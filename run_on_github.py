...
# --- 2. YOUTUBE VIDEO DOWNLOADER (YT-DLP WITH SAFARI/IOS CLIENT BYPASS) ---
def download_youtube_video(video_url, output_path):
    """Downloads YouTube Video or Shorts safely in MP4 format using Safari Web client bypass."""
    ydl_opts = {
        'format': 'best[ext=mp4]/best',
        'outtmpl': output_path,
        'quiet': True,
        'no_warnings': True,
        # BYPASS YOUTUBE BOT DETECTOR: Force emulating official Safari and iOS App Clients
        'extractor_args': {
            'youtube': {
    ...
    for idx, rule in enumerate(rules):
        ...
        for source_id in source_ids:
            try:
                    ...
                    for entry in reversed(feed.entries[:15]):
                        ...
                        try:
                            ...
                            if web_res.status_code == 200:
                                ...
                                if not og_match:
                                    og_match = re.search(r'<meta[^>]+content=["\']([^"\']+)["\']', web_res.text, re.IGNORECASE)
                ...
                # --- C. YOUTUBE SOURCE AUTOMATION (Uses native RSS XML under the hood with Safari client bypass) ---
